...
class AnalyticsController implements GlobalController {
    ...
    private initializeRoutes() {
        ...
        this.router.get("/volatility/:productId", [authenticate], this.getVolatility);
    }

    private getTrend = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const period = Number(req.query.period) || 30;
            const trend = await this.analyticsService.getTrend(req.params.productId, period);
            res.status(200).json({ status: "Success", payload: trend });
        } catch (error) { next(error); }
    }

    private getCompare = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const products = (req.query.products as string || "").split(",").filter(Boolean);
            const marketId = req.query.market as string;
            if (!marketId) {
                res.status(400).json({ status: "Failed", message: "Market ID is required" });
                return;
            }
            const period = Number(req.query.period) || 30;
            const compare = await this.analyticsService.getCompare(products, marketId, period);
            res.status(200).json({ status: "Success", payload: compare });
        } catch (error) { next(error); }
    }

    private getMovers = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const marketId = req.query.market as string;
            if (!marketId) {
                res.status(400).json({ status: "Failed", message: "Market ID is required" });
    ...
    private getVolatility = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const period = Number(req.query.period) || 30;
            const volatility = await this.analyticsService.getVolatility(req.params.productId, period);
            res.status(200).json({ status: "Success", payload: volatility[0] || null });
        } catch (error) { next(error); }
    }
}
