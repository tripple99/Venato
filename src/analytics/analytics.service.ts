...
class AnalyticsService {
  ...
  public async getCompare(productIds: string[], marketId: string, periodDays: number = 30) {
    try {
      const periodDate = new Date();
      periodDate.setDate(periodDate.getDate() - Math.max(1, periodDays));
      const objectIdProducts = productIds.map(id => new mongoose.Types.ObjectId(id));
      ...
      throw new HttpException(500, "Failed", "Could not fetch comparison analytics");
    }
  }
  ...
  public async getMovers(marketId: string, periodDays: number = 7) {
    try {
      ...
      return await priceSnapshotModel.aggregate([
           ...
           $addFields: {
              ...
              percentageChange: {
                  $cond: [
                      { $eq: ["$oldestPrice", 0] },
                      0,
                      { $multiply: [{ $divide: [{ $subtract: ["$newestPrice", "$oldestPrice"] }, "$oldestPrice"] }, 100] }
                  ]
              }
           }
        },
       ...
       throw new HttpException(500, "Failed", "Could not fetch movers analytics");
    }
  }
  ...
  public async getVolatility(productId: string, periodDays: number = 30) {
    try {
      ...
      return await priceSnapshotModel.aggregate([
       ...
       throw new HttpException(500, "Failed", "Could not fetch volatility analytics");
    }
  }
}
