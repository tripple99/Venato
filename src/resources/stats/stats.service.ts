  ...
  public async getStats(role: AuthRole, userId: string): Promise<StatsResult> {
    try {
      ...
      if (role === AuthRole.User) {
        const [watchlistCount, activeAlerts, inventoryStats] =
          await Promise.all([
                ...
                $addFields: {
                  ...
                  productUnitFactor: {
                    $switch: {
                      branches: [
                        {
                          case: { $eq: ["$productInfo.unit", "mudu"] },
                          then: 1.25,
                        },
                        {
                          case: { $eq: ["$productInfo.unit", "tiya"] },
                          then: 0.125,
                        },
                        {
                          case: { $eq: ["$productInfo.unit", "litre"] },
                          then: 0.9,
                        },
                        {
                          case: { $eq: ["$productInfo.unit", "kg"] },
                          then: 1.0,
                        },
                        {
                          case: { $eq: ["$productInfo.unit", "tonne"] },
                          then: 1000.0,
                        },
                      ],
                      default: 1.0,
