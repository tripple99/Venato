...
class InventoryService {
  public async create(
    ...
    try {
      ...
      const updatedInventory = await inventoryModel
        ...
        .populate("preferredMarket", "name location");
  ...
  public async getPortfolioPerformance(
          ...
          $addFields: {
            ...
            productUnitFactor: {
              $switch: {
                branches: [
                  {
                    case: { $eq: ["$productDetails.unit", "mudu"] },
                    then: 1.25,
                  },
                  {
                    case: { $eq: ["$productDetails.unit", "tiya"] },
                    then: 0.125,
                  },
                  {
                    case: { $eq: ["$productDetails.unit", "litre"] },
                    then: 0.9,
                  },
                  { case: { $eq: ["$productDetails.unit", "kg"] }, then: 1.0 },
                  {
                    case: { $eq: ["$productDetails.unit", "tonne"] },
                    then: 1000.0,
                  },
                ],
                default: 1.0,
    ...
    return {
      totalPortfolioValue,
      holdings: createPaginatedResult(
        inventories,
        totalCount,
        pagination.page,
        pagination.limit,
