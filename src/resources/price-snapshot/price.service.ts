...
class PriceSnapshotService {
  ...
  public async create(
    priceSnapshot: IPriceSnapshotCreate,
  ): Promise<IPriceSnapshot> {
    try {
      const createdPriceSnapshot =
        await priceSnapshotModel.create(priceSnapshot);
  ...
  public async fetch(priceSnapshot: IPriceSnapshot): Promise<IPriceSnapshot> {
    try {
      const fetchedPriceSnapshot = await priceSnapshotModel.findById(
        priceSnapshot.id,
      );
  ...
  public async update(
    id: string,
    priceSnapshot: Partial<IPriceSnapshot>,
  ): Promise<IPriceSnapshot> {
    try {
      const updatedPriceSnapshot = await priceSnapshotModel.findOneAndUpdate(
        { productId: id },
        priceSnapshot,
        { new: true },
      );
      if (!updatedPriceSnapshot)
        throw new HttpException(404, "Not found", "Price snapshot not found");
      await this.alertService.processMatchingAlerts(updatedPriceSnapshot);
