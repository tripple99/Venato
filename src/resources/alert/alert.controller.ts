...
class AlertController implements GlobalController {
  ...
  private intiailizedRoutes(): void {
    this.router.post(
      "/",
      [authenticate, schemaValidator(alertValidators.create)],
      this.createAlert,
    );
    ...
    this.router.get(
      "/suggest/:productId",
      [authenticate],
      this.suggestThresholds,
    );
    this.router.get("/:id", [authenticate], this.getAlertById);
    this.router.patch(
      "/:id",
      [authenticate, schemaValidator(alertValidators.updateAlert)],
      this.updateAlert,
    );
  ...
  createAlert = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!market)
        throw new HttpException(404, "Not found", "market not found");
